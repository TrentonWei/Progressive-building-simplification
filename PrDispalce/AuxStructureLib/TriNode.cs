...
namespace AuxStructureLib
{
    ...
    { 
          public int InOut;//考虑到建筑物图形Hole的问题，InOut=0表示是外轮廓；InOut>0表示是洞
       // public double X;        //坐标
       // public double Y;        //坐标
       // public int ID;         //编号
        public int TagValue;   //标识值,1、道路网：当为端点是为-1，否则为所属线目标的ID(所关联的目标编号)
        /// <summary>
        /// 要素类型
        /// </summary>
        ...
        /// <returns></returns>
        public double ReturnDist(List<TriNode> pointList, bool bol)
        {
            double MiniDist = 100000;    //总体最小距离
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                double minidist = 0.0;
                //多义线的分线段的两个端点
                TriNode st = pointList[i]; TriNode ed = pointList[i + 1];
        ...
        */
        ...
        /// 通过ID号获取triNode目标
        /// </summary>
        /// <param name="PLList">triNode列表</param>
        /// <param name="ID">ID号</param>
        /// <returns></returns>
        public static TriNode GetTriNodebyID(List<TriNode> NList, int ID)
        {
            foreach (TriNode curP in NList)
            {
                if (curP.ID == ID)
                    return curP;
            }
        ...
        public static void Create_WriteVetex2Shp(string filePath, string fileName, List<TriNode> TriNodeList, ISpatialReference pSpatialReference)
        {
